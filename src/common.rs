...
// TODO refactor base Vec<Address> into wrapper struct
...
impl<'a> Deser<'a> for Message {
    fn deser_from_iter<I>(stream: &mut I) -> Result<Self, DeserError>
    where
        I: Iterator<Item = u8>,
    {
        ...
        let code = if let Some(s) = code {
            ...
            return Err(DeserError::Message(
                "Missing message code byte.".into(),
            ));
        };
        match code {
            MESSAGE_PING_CODE => {
                let addresses = Vec::<Address>::deser_from_iter(stream)?;
                Ok(Message::Ping(addresses))
            }
            MESSAGE_REQUEST_POST_CODE => {
                let hash = U256::deser_from_iter(stream)?;
                Ok(Message::RequestPost(hash))
            }
            MESSAGE_SHARE_POST_CODE => {
                let post = Post::deser_from_iter(stream)?;
                Ok(Message::SharePost(post))
            }
            _ => Err(DeserError::Message("Unknown message code.".into())),
        }
    }
}
...
mod tests {
    ...
    // TODO test Message RequestPost
    // TODO test Message SharePost
}
